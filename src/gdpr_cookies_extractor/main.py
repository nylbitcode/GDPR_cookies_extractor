...
async def process_site_scenario(
    ...
    try:
        ...
        if 'find-pp' in tasks and not result.privacy_policy_url:
            ...
            policy_result, _ = await analyzer.find_privacy_policy(
                page, site_dump_folder,
                filter_keywords=search_keywords_config.get('privacy_policy', []),
            )
            
            if not analyzer.no_llm:
                result.update_llm_output(policy_result)

            if policy_result.get("privacy_policy_url"):
                result.privacy_policy_url = urljoin(page.url, policy_result.get("privacy_policy_url"))
