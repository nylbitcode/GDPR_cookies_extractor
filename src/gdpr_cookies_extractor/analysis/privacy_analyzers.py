...
import re
import os
import asyncio  # Aggiunto per gestire il fan-out parallelo
...
class PrivacyAnalyzer:
    ...
    async def _dump_snapshot(self, page, site_dump_folder: str, phase: str, all_links: List[ExtractedLink]):
        ...
        try:
            ...
            logger.debug(f"Dumped snapshot for phase '{phase}' to {site_dump_folder}")
    ...
    async def _extract_policy_url_from_html(self, html_content: str, url: str, promising_links: List[str]):
        ...
        **CRITICAL RULE: If the candidate link list is not empty, you choose the best and most relevant option from that list. Only if the candidates list is empty you can search in the HTML content.** When searching, look for links containing keywords like 'privacy policy', 'GDPR', 'data protection', 'privacy center'.
        ...
        try:
            ...
            if page.url != url:
                try:
                    ...
                    return {"privacy_policy_url": None, "reasoning": "Navigation failed"}, []
            ...
            # Check for external redirect after navigation using the provided root domain logic
            final_netloc = urlparse(page.url).netloc
            # Simple check: is the original root inside the final netloc?
            # This handles cases like 'example.com' redirecting to 'www.example.com' or 'shop.example.com'
            if original_root_domain not in final_netloc and final_netloc not in original_root_domain:
                 logger.warning(f"Redirected to potentially external domain: {page.url} (Original root: {original_root_domain}). Proceeding with caution.")
                 # We don't abort immediately as some redirects are valid (e.g. auth portals), but it's a signal.
            ...
            # --- CORREZIONE ERRORE 3: Validazione più rigorosa ---
            if promising_links_objects and llm_url:
                # Normalizziamo l'URL LLM e i candidati per il confronto
                llm_url_clean = llm_url.strip().rstrip('/')
                
                is_llm_choice_valid = False
                for link_obj in promising_links_objects:
                    cand_href_clean = link_obj.href.strip().rstrip('/')
                    # Controlliamo match esatto o se l'LLM ha dato un path relativo corretto
                    if llm_url_clean == cand_href_clean or cand_href_clean.endswith(llm_url_clean):
                        is_llm_choice_valid = True
                        break
                
                if not is_llm_choice_valid:
                    logger.warning(f"LLM choice '{llm_url}' failed strict validation against candidates. Applying heuristic fallback.")
                    ...
                    if heuristic_url:
                        ...
                        policy_output["reasoning"] = "LLM choice overridden by heuristic due to non-compliance."
                        policy_output["confidence_score"] = 0.95
                    else:
            ...
            if page and not page.is_closed():
                # We do NOT close the page here if it was passed from outside for reusing context, 
                # but given the architecture, we close it to be safe if created locally. 
                # In this refactor, pages are created in the orchestrator.
    ...
    async def find_privacy_policy(self, context, site_url: str, site_dump_folder: str, filter_keywords: Optional[List[str]] = None) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
        ...
        Orchestrates the search for the privacy policy URL with FAN-OUT support.
        """
        found_policies = []
        link_extraction_phases = []
        ...
        try:
            ...
            # More robust root domain extraction
            root_domain = base_netloc
            ...
            # --- CORREZIONE ERRORE 1: Fan-Out Logic ---
            # Se abbiamo link promettenti dalla scansione iniziale, li visitiamo in parallelo
            if initial_links:
                # Estraiamo i link promettenti dall'ultima fase di analisi
                last_phase = initial_links[-1]
                candidates = last_phase.get("promising_extracted_links", [])
                
                # Filtriamo il sito stesso per evitare loop e limitiamo a 3 candidati
                valid_candidates = [u for u in candidates if u != site_url][:3]
                
                if valid_candidates:
                    logger.info(f"Fan-out: Analyzing {len(valid_candidates)} candidate pages concurrently...")
                    tasks = []
                    # Creiamo le pagine in anticipo
                    for i, cand_url in enumerate(valid_candidates):
                        new_p = await context.new_page()
                        tasks.append(
                            self._analyze_page_for_policy(
                                new_p, cand_url, site_dump_folder, i + 1, root_domain, filter_keywords
                            )
                        )
                    ...
                    for res in fan_out_results:
                        if isinstance(res, Exception):
                            logger.error(f"Fan-out task failed: {res}")
                            continue
                            
                        policy_res, links_res = res
                        link_extraction_phases.extend(links_res)
                        if policy_res and policy_res.get("privacy_policy_url"):
                            found_policies.append(policy_res)
    ...
    async def _extract_cookie_link_from_html(self, html_content: str, url: str, promising_links: List[str]) -> Dict[str, Any]:
        ...
        **CRITICAL RULE: If the candidate link list is not empty, you MUST choose the best and most relevant option from that list. Only if the candidates list is empty you can search in the full HTML content.** The privacy policy and cookie policy are often separate. I am on the privacy page, and I need to find the link to the specific cookie policy page.
    ...
    async def find_cookie_declaration_page(self, context, privacy_policy_url: str, site_dump_folder: str, search_keywords_config: Dict[str, List[str]]) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
        ...
        try:
            ...
            if llm_chosen_link:
                 for link_obj in promising_links_objects:
                    if llm_chosen_link == link_obj.href or link_obj.href.endswith(llm_chosen_link):
                        is_llm_choice_valid = True
                        break
            ...
            logger.error(f"Error during multi-stage cookie declaration search: {e}")
            if stage1_result:
                return stage1_result, link_extraction_phases
            return {"cookie_declaration_url": None, "reasoning": f"An exception occurred: {e}"}, link_extraction_phases
        finally:
            if page: await page.close()
            if validation_page: await validation_page.close()
    ...
    async def _ask_llm_about_data_retention_declaration(self, page_content: str) -> Dict[str, Any]:
        ...
        """
        prompt = f"""
        You are an expert in GDPR and web compliance. Your task is to analyze the following text to determine if it contains a "Data Retention" policy.

        Look for headings like "Data Retention", "How long we keep your data", or "Retention of Personal Information".
        Extract a concise summary of the data retention periods.
        ...
        response = await self.llm_client.query_json(user_prompt=prompt)
        if not response.success:
            return {"has_data_retention_declaration": False, "reasoning": response.error, "retention_period_summary": None}
        return response.data

    async def _extract_data_retention_link_from_html(self, html_content: str, url: str, promising_links: List[str]) -> Dict[str, Any]:
        prompt = f"""
        Find a URL pointing to a "Data Retention Policy" or "Data Storage Information" page from the HTML of {url}.
        Candidate links: {promising_links}.
        ...
        response = await self.llm_client.query_json(user_prompt=prompt)
        if not response.success:
             return {"data_retention_policy_link": None, "reasoning": response.error, "confidence_score": 0.0}
        return response.data
    ...
    async def find_data_retention_page(self, context, privacy_policy_url: str, site_dump_folder: str, search_keywords_config: Dict[str, List[str]]) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
        ...
        try:
            ...
            if not promising_links_objects:
                if stage1_result: return stage1_result, link_extraction_phases
            ...
            if llm_chosen_link and is_llm_choice_valid:
                ...
                if heuristic_choice: final_candidate_url = heuristic_choice
                else:
                    if stage1_result: return stage1_result, link_extraction_phases
            ...
            if validation_llm_result.get("has_data_retention_declaration"):
                ...
                if stage1_result: return stage1_result, link_extraction_phases
                return {"data_retention_url": None, "reasoning": f"Found link {full_candidate_url}, but content validation failed."}, link_extraction_phases

        except Exception as e:
            logger.error(f"Error during data retention page search: {e}")
            if stage1_result: return stage1_result, link_extraction_phases
    ...
    async def _ask_llm_about_data_deletion_declaration(self, page_content: str) -> Dict[str, Any]:
        prompt = f"""
        Analyze the following text to determine if it contains a "Data Deletion" policy.
        Extract a concise summary of how a user can delete their data.
        ...
        response = await self.llm_client.query_json(user_prompt=prompt)
        if not response.success:
             return {"has_data_deletion_declaration": False, "reasoning": response.error, "deletion_method_summary": None}
        return response.data

    async def _extract_data_deletion_link_from_html(self, html_content: str, url: str, promising_links: List[str]) -> Dict[str, Any]:
        prompt = f"""
        Find a URL pointing to "Data Deletion", "Privacy Dashboard", or "Manage Your Data" page from the HTML of {url}.
        Candidate links: {promising_links}.
        ...
        response = await self.llm_client.query_json(user_prompt=prompt)
        if not response.success:
             return {"data_deletion_policy_link": None, "reasoning": response.error, "confidence_score": 0.0}
        return response.data
    ...
    async def find_data_deletion_page(self, context, privacy_policy_url: str, site_dump_folder: str, search_keywords_config: Dict[str, List[str]]) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
        ...
        try:
            ...
            if validation_llm_result.get("has_data_deletion_declaration"):
                ...
                if stage1_result: return stage1_result, link_extraction_phases
                return {"data_deletion_url": None, "reasoning": f"Found link {full_candidate_url}, but content validation failed."}, link_extraction_phases

        except Exception as e:
            logger.error(f"Error during data deletion page search: {e}")
            if stage1_result: return stage1_result, link_extraction_phases
    ...
    async def _ask_llm_about_dpo_declaration(self, page_content: str) -> Dict[str, Any]:
        prompt = f"""
        Analyze the following text to determine if it contains contact information for a Data Protection Officer (DPO).
        Extract a concise summary of the contact methods.
        ...
        response = await self.llm_client.query_json(user_prompt=prompt)
        if not response.success:
             return {"has_dpo_declaration": False, "reasoning": response.error, "dpo_contact_summary": None}
        return response.data

    async def _extract_dpo_link_from_html(self, html_content: str, url: str, promising_links: List[str]) -> Dict[str, Any]:
        prompt = f"""
        Find a URL pointing to "Data Protection Officer", "Privacy Contact", or "Data Controller" page from the HTML of {url}.
        Candidate links: {promising_links}.
        ...
        response = await self.llm_client.query_json(user_prompt=prompt)
        if not response.success:
             return {"dpo_policy_link": None, "reasoning": response.error, "confidence_score": 0.0}
        return response.data
    ...
    async def find_dpo_page(self, context, privacy_policy_url: str, site_dump_folder: str, search_keywords_config: Dict[str, List[str]]) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
        ...
        try:
            ...
            if validation_llm_result.get("has_dpo_declaration"):
                ...
                if stage1_result: return stage1_result, link_extraction_phases
                return {"dpo_url": None, "reasoning": f"Found link {full_candidate_url}, but content validation failed."}, link_extraction_phases

        except Exception as e:
            logger.error(f"Error during DPO page search: {e}")
            if stage1_result: return stage1_result, link_extraction_phases
    ...
    async def categorize_cookies(self, cookies_data: list) -> List[CookieCategory]:
        ...
        - "Functional": Remembers user choices.
        - "Analytical": Collects data on user behavior.
        ...
        The value of "cookie_categories" must be a list of objects (one for each category).
        Each category object must contain "category_name" and "cookies" (list).
        Each cookie object must contain "name", "domain", "description".
    ...
    async def _extract_all_internal_links(self, page) -> List[ExtractedLink]:
        ...
        base_netloc = urlparse(site_url).netloc
        # --- CORREZIONE ERRORE 2: Logica dominio più robusta ---
        if ":" in base_netloc:
             base_netloc = base_netloc.split(":")[0]
        
        # Determine strict root domain for matching
        root_domain = base_netloc
        if root_domain.startswith("www."):
            root_domain = root_domain[4:]
        ...
        for a in await page.query_selector_all('a'):
            ...
            try:
                ...
                if href:
                    ...
                    if ":" in link_netloc:
                        link_netloc = link_netloc.split(":")[0]
                    
                    # More permissive internal check: 
                    # check if the link netloc ends with root domain OR root domain ends with link netloc
                    # This covers:
                    # example.com -> shop.example.com
                    # shop.example.com -> example.com
                    is_internal = (link_netloc == root_domain) or \
                                  link_netloc.endswith("." + root_domain) or \
                                  root_domain.endswith("." + link_netloc)
                    
                    if is_internal and '#' not in full_url and not full_url.endswith(('.js', '.css', '.png', '.jpg', '.jpeg', '.gif', '.pdf', '.xml', '.json', '.zip', '.rar', '.tar', '.gz', '.svg', '.ico')):
